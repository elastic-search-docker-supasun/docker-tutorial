...
const PATH_INSPECT = process.env.PATH_INSPECT;
...
processMonitor.on('change', () => {
  ...
  var usedSpace = Number(((statsSync.disks[PATH_INSPECT].percent)*100).toFixed(8)); //Convert to Percent
  var freeSpace = Number(((statsSync.disks[PATH_INSPECT].free)/1024).toFixed(8)); //Convert to Percent
  ...
  if (SH_OS=="Y"){
    ...
    osMSG=osMSG+"Free Space in '/': "+freeSpace+" MB\nUsed Space in '/': "+usedSpace+" %";
  }
  else {
    sendNotify="N";
